...
const SearchGroup = () => {
  ...
  const filteredGroups = searchGroupMock.Page.content.filter(group =>
    activeFilters.length === 0 || group.tagList.some(tag => activeFilters.includes(tag.tagId)) ||
  ...
  return (
    <Wrapper>
      <Title>그룹 탐색</Title>
      <SearchContainer>
        <SearchBar placeholder="홈트를 함께 이어나갈 그룹을 검색해보세요" onChange={handleSearchChange} value={searchTerm}/>
        <SearchIcon onClick={handleSearch} className='material-symbols-outlined' isActive={false}>search</SearchIcon>
      </SearchContainer>
      <Filters>
      ...
      <GroupContainer>
        {filteredGroups.map((team: Team) => (
          <GroupCard key={team.teamName}>
            ...
            <MemberInfo>멤버 {team.currentParticipants}/{team.maxParticipants}명</MemberInfo>
